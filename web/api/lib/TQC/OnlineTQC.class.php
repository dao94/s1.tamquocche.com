<?php
class OnlineTQC extends Online {
	//参数转换
	function __construct() {
		//处理url参数, 各个代理的参数可能不同 特别是特殊代理 转换成内部统一参数
		$makeSign = array();
		$makeSign['time'] = $this->params['time'] = isset($_REQUEST['time']) ? intval($_REQUEST['time']) : 0;
		$this->params['start_time']=isset($_REQUEST['start_time']) ? my_escape_string($_REQUEST['start_time']) : 0;
		isset($_REQUEST['end_time']) ? $this->params['end_time']=my_escape_string($_REQUEST['end_time']) : '';
