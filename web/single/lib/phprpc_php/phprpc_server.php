...
class PHPRPC_Server {
	...
	function callFunction() {
		##################密钥认证################################
		$flag = md5($_REQUEST['phprpc_id'] . PHPRPC_KEY . $_REQUEST['phprpc_time'] . $_REQUEST['phprpc_func']);
		//校验码不正确或者js端调用时链接超时（主要是防止保存链接来破解接口）而php端调用时主要是会出现经常Sửa服务器时间等问题，而不增加时间校验避免影响登陆等接口
		if ($flag !=$_REQUEST['phprpc_sign']) {
			$this->errno = E_ERROR;
