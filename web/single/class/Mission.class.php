...
class Mission{
	...
	public function __construct(){
		...
		$bin_conf_filename=dirname(dirname(dirname(__ROOT__)))."/server_bin/{$version}/bin/config/xml/mission/quest.xml";//旧路径
		$bin_conf_filename_new=dirname(dirname(dirname(dirname(__ROOT__))))."/server_bin/{$version}/bin/config/xml/mission/quest.xml";//新路径
		if(is_file($bin_conf_filename)){
			...
			$this->conf_filename=__XML__.'quest.xml';
		}
	...
	public function getList(){
		...
		if($file_time>$cache_time || !$cache_time){
	...
	private function getListByXml(){
		...
		while ($reader->read()){
			if($reader->localName=='Quest' && $reader->nodeType==XMLReader::ELEMENT){
