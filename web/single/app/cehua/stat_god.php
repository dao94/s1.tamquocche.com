...
$sql="select count(distinct date) as count from stat_god $where ";
$mysqli=new DbMysqli();
$total_rows=$mysqli->count($sql);
$p=new Page($total_rows);
$sql="select * from stat_god $where order by date desc limit {$p->firstRow},{$p->listRows} ";
$query=$mysqli->query($sql);
$data=array();
while ($row=$query->fetch_assoc()){
	$row['week']=date('N',strtotime($row['date']));
	$row['shenhua_remark']=json_decode($row['shenhua_remark'],true);
	$data[]=$row;
}
$page=$p->show();
