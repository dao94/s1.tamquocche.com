...
$sort_fields=array('char_id','item_id','item_num','money_type','price','total_money','time');
...
$list_row=empty($_GET['list_row']) ? 20 : intval($_GET['list_row']);
$sort_field=empty($_GET['sort_field']) ? '' : my_escape_string(trim($_GET['sort_field']));
$sort_field=in_array($sort_field, $sort_fields) ? $sort_field : 'time';//排序字段
$sort_flag=empty($_GET['sort_flag']) ? 0 : intval($_GET['sort_flag']);//排序1=升序 0=降序
$sort=$sort_flag ? 'asc' : 'desc';
$conditions=array(
...
$sql="select *,price*item_num as total_money from log_consignment $where order by $sort_field $sort limit {$p->firstRow},{$p->listRows}";
...
$smarty->assign('list_row_config',range(20, 200,20));
