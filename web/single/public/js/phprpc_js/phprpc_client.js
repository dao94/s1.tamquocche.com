        ...
        function PHPRPC_Client(serverURL, functions) {
            ...
            function getSign(extra_url){
                ...
                xmlhttp.onreadystatechange = function() {
                    if (xmlhttp.readyState == 4 && !xmlhttpDone) {
                        xmlhttpDone = true;
                        phprpc_sign = xmlhttp.responseText;
                        xmlhttp = null;
                    }
                }
                xmlhttp.open('POST',_getBasePath()+'makeSign.php',false);
                xmlhttp.setRequestHeader("Content-type","application/x-www-form-urlencoded");
                xmlhttp.send(extra_url);
                return phprpc_sign;
            }
            function _getBasePath() {
                ...
                for (var i = 0, len = els.length; i < len; i++) {
                    src = els[i].src || '';
                    if (/phprpc_client[\w\-\.]*\.js/.test(src)) {
                        return src.substring(0, src.lastIndexOf('/') + 1);
                    }else if(/lwjsback[\w\-\.]*\.js/.test(src)){
                        return src.substring(0, src.lastIndexOf('/') + 1)+'phprpc_js/';
                    }else if(/kindeditor[\w\-\.]*\.js/.test(src)){
                        return src.substring(0, src.lastIndexOf('/') + 1)+'../phprpc_js/';
                    }
                }
                return '';
            ...
            function call(id, func, args) {
                ...
                var request = 'phprpc_args=' + btoa(encryptString(m_php.serialize(args), encrypt, 1))
                + '&phprpc_encode=false'
                + '&phprpc_encrypt=' + encrypt;
            
                var phprpc_id = m_preID + m_clientID;
                var phprpc_time = Math.floor((new Date()).getTime()/1000);
                var extra_url = '&phprpc_id=' + phprpc_id + '&phprpc_time='+ phprpc_time +'&phprpc_func='+func+'&';
                request += extra_url;
                request += 'phprpc_sign=' +getSign(extra_url)+'&';
